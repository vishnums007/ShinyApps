...
server <- function(input, output, session) { 
  ...
  output$view_stocks_ui<-renderUI({
    ...
    fluidPage(
      ...
      fluidRow(
        wellPanel(
          h4(strong(textOutput("Fishroom_title"))),
          br(),
          dataTableOutput("fishroom_map",width = "auto",height = "auto")%>% withSpinner(type=5)
        )
      )
    )
  })
  ...
  room_map<-eventReactive(input$fishroom_upload,{
    if (fishroom_choice()=="Johnson Fish Room"){
      zebra<- read.csv(file = "Johnson_room.csv", header = T, fill = T)
    }else if(fishroom_choice()=="Streisinger Fish Room"){
      zebra<- read.csv(file = "Streisinger_room.csv", header = T, fill = T)
    }else if(fishroom_choice()=="Walker Nursery"){
      zebra<- read.csv(file = "Walker nursery.csv", header = T, fill = T)
    }else if(fishroom_choice()=="Johnson Nursery"){
      zebra<- read.csv(file = "Johnson nursery .csv", header = T, fill = T)
    }
  })
    ...
    fluidPage(
      fluidRow(
        box(width = 12, collapsible = TRUE, title = strong("Instructions:"), "
            ...
            Johnson, Streisinger and Nursery rooms. First select whether you want to add larval fish
            or Adult Fish. Then, fill out the respective form shown in the given tab and
        hit submit to add stocks. Then, you will be asked to verify the information you provided and
        click on the confirm button to add your stock to the database. Once you hit confirm, your 
        ...
        tabPanel("Add Larvae",
                 div(id="larval_addition",
                     wellPanel(
                       ...
                       dateInput(inputId = "DOB_add_n",label = "Date of birth* :", max = Sys.Date()),
        ...
        tabPanel("Add Adult Fish",
                 div(id="adult_addition",
                     wellPanel(
                       ...
                       dateInput(inputId = "DOB_add_adult",label = "Date of birth* :", max = Sys.Date()),
                       ...
                       selectizeInput(inputId = "Experiment_label_adult", label = "Experiment label*", choices= NULL ,multiple = T,options = list(create=F, placeholder = "Select Experiment label")),
  ...
  observeEvent(input$nursery_add_select,{
    archive<-read.csv(file = "Archive.csv", header = T, fill = T)
  ...
  observeEvent(input$Add_nursery,{
    ...
    if (nursery_add_choice()=="Walker Nursery"){
      zebra1<- read.csv(file = "Walker nursery.csv",header = T, fill = T)
      
    }else if(nursery_add_choice()=="Johnson Nursery"){
      zebra1<- read.csv(file = "Johnson nursery .csv",header = T, fill = T)
  ...
  output$nursery_add_conf_data<-renderTable({
    ...
    features[4,]<-c("Date of Birth :", as.character(input$DOB_add_n))
  ...
  observeEvent(input$Add_nursery_confirm, {
    ...
    log_data<-read.csv(file = "log.csv",header = T, fill = T)
    
    archive[nrow(archive)+1,]<-list(input$nTanks_add_n,input$Stock_no_add_n,as.character(input$DOB_add_n),input$Fish_name_add_n,
                                    ...
                                    input$Experiment_n)
    log_data[nrow(log_data)+1,]<-list( format(Sys.time(), "%d-%b-%Y %H.%M"),input$Stock_no_add_n,as.character(input$DOB_add_n),input$Fish_name_add_n,
    ...
    zebra1[nrow(zebra1)+1,]<-list(input$nTanks_add_n,input$Stock_no_add_n,as.character(input$DOB_add_n),input$Fish_name_add_n,
                                  input$Fish_Genotype_add_n,input$Fish_Responsible_add_n,input$nFish_add_n,
                                  input$Experiment_n)
  ...
  observeEvent(input$upload_adult_room,{
    if(adult_add_choice()=="Johnson Fish Room"){
      adult_file <- read.csv(file = "Johnson_room.csv", header = T, fill = T)
    }else if (adult_add_choice()=="Streisinger Fish Room") {
      adult_file <- read.csv(file = "Streisinger_room.csv", header = T, fill = T)
    }
    ...
    exp_label<-list("GENOTYPE","SURGERY","BREEDING STOCK","TRANSPLANT","OTHERS")
  ...
  observeEvent(input$Add_adult_Fish_stock,{
    user_input<- c(input$Tank_name_adult,input$Stock_no_add_adult,as.character(input$DOB_add_adult),input$Fish_name_add_adult,
                   ...
                   input$Experiment_label_adult,input$Food_label_adult
    )
    qc<-check_errors(user_input)
    if(qc==T){
  ...
  output$adult_add_conf_data<-renderTable({
    ...
    featuresa[4,]<-c("Date of Birth :", as.character(input$DOB_add_adult))
    ...
    featuresa[9,]<-c("Experimental Label :", input$Experiment_label_adult)
  ...
  observeEvent(input$Add_adult_confirm,{
    ...
    adult_file[locate_tank(adult_file,input$Tank_name_adult),]<-list(input$Tank_name_adult, input$Stock_no_add_adult, as.character(input$DOB_add_adult),
                                                                     input$Fish_name_add_adult, input$Fish_Genotype_add_adult, input$Fish_Responsible_add_adult,
                                                                     input$nFish_add_adult,input$Experiment_label_adult,input$Food_label_adult,
                                                                     input$Notes_add_adult)
    log_data<-read.csv(file = "log.csv",header = T, fill = T)
    
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),input$Stock_no_add_adult,as.character(input$DOB_add_adult),
                                      input$Fish_name_add_adult,input$Fish_Genotype_add_adult,input$Fish_Responsible_add_adult,
    ...
    fluidPage(
      fluidRow(
        box(width = 12, collapsible = TRUE, title = strong("Instructions:"), "
           ...
           needs to be transferred. Then, you can type in stock number/fish name/tank location to select the stock. Then you need to load
           the fish room to which the stock is transferred. You can change the fish number, Genotype, Experiment label and Food label
           at the location where it is transferred.Once you submit the form, you will be asked to verify your action and then click confirm. 
        ...
        tabPanel("From Nursery to Fish Rooms",
                 div(id="transfer fish_nursery",
                     wellPanel(
                       ...
                       br(),
                       fluidRow(column(width=12, dataTableOutput("nursery_stock_transfer_quickview", width = "auto",height = "auto"))),
                       br(),
                       br(),
                       ...
                       selectizeInput(inputId = "Experiment_label_nursery_transfer", label = "Experiment label*", choices= NULL ,multiple = T,options = list(create=F, placeholder = "Select Experiment label")),
        ...
        tabPanel("Within Fish Rooms",
                 div(id="transfer fish_adult",
                     wellPanel(
                       ...
                       hr(),
                       fluidRow(column(width=12, dataTableOutput("adult_stock_transfer_quickview", width = "auto",height = "auto"))),
                       br(),
                       h3(strong("FROM :-")),
                       hr(),
                       ...
                       selectizeInput(inputId = "Experiment_label_adult_transfer", label = "Experiment label*", choices= NULL ,multiple = T,options = list(create=F, placeholder = "Select Experiment label")),
  ...
  observeEvent(input$nursery_transfer_upload,{
    if (nursery_transfer_room()=="Walker Nursery"){
      zebra_n<- read.csv(file = "Walker nursery.csv", header = T, fill = T)
      zebra_ad<- read.csv(file = "Streisinger_room.csv", header = T, fill = T)
      ...
      zebra_n<- read.csv(file = "Johnson nursery .csv", header = T, fill = T)
      zebra_ad<- read.csv(file = "Johnson_room.csv", header = T, fill = T)
      output$nursery_transfer_room_selection<- renderText({
    ...
    exp_label<-list("GENOTYPE","SURGERY","BREEDING STOCK","TRANSPLANT","OTHERS- Mention in notes")
  ...
  observeEvent(input$nursery_Fish_transfer_submit,{
    ...
    user_input<- c(input$Select_nursery_stock_transfer, input$Select_destination_tank_nursery_transfer, input$nursery_transfer_n,
                   input$Experiment_label_nursery_transfer, input$Food_label_adult_nursery_transfer, input$Notes_nursery_trasnfer
    )
    qc<-check_errors(user_input)
    if(qc==T){
  ...
  observeEvent(input$adult_stock_transfer_source_submit,{
    ...
    user_input<-c(input$adult_stock_transfer_source_name, input$adult_stock_transfer_destination_name, input$adult_transfer_n,
                  input$genotype_edit_adult_transfer, input$Experiment_label_adult_transfer, input$Food_label_adult_adult_transfer,
  ...
  observeEvent(input$adult_Fish_transfer_confirm,{
    ...
    zebra_ad[to_stock_loc, ]<-list(
      ...
      input$genotype_edit_adult_transfer, zebra_ad[from_stock_loc,6], input$adult_transfer_n, input$Experiment_label_adult_transfer,
  ...
  output$edit_stock_ui<-renderUI({
    ...
    fluidPage(
      fluidRow(
        box(width = 12, collapsible = TRUE, title = strong("Instructions:"), "
            Here, you will be able to edit or euthanize any stocks present in four Fish rooms. First you have to load the Fish room from which the stock 
           is present. Then, you can type in stock number (in case of nursery fish)/tank location (in case of adult fish) and select the stock. You can change the fish number, Genotype, Experiment label, Food label
           and notes.Once you submit the form, you will be asked to verify your action and then click confirm. 
           Once you confirm, you actions will be logged. You can edit another fish by hitting the reset button. If you click on the Euthanize button, the selected number of fish
      ...
      wellPanel(
        ...
        div(id="edit_fish_room_options",
            ...
            hr(),
            dataTableOutput("edit_fish_room_quickview", width = "auto",height = "auto"),
        ...
        div(id= "edit_adult_room",
            ...
            div(id="edit_Adult_stocks",
                ...
                selectizeInput(inputId = "ExpLabel_edit_Afish_target", label = "Experiment Label*", choices= NULL ,multiple = F,options = list(create=F, placeholder = "Edit experiment label")),
  ...
  observeEvent(input$edit_Afish_target_submit,{
    ...
    track_change[3,]<- c(zebra_ad[adt_location,8] ,input$ExpLabel_edit_Afish_target)
    ...
    user_input_values<- c(input$genotype_edit_Afish_target,input$Number_edit_Afish_target,input$ExpLabel_edit_Afish_target,input$FoodLabel_edit_Afish_target,
  ...
  observeEvent(input$AFish_edit_confirm,{
    ...
    zebra_ad[adt_location,8] <- input$ExpLabel_edit_Afish_target
      ...
      fluidRow(
        ...
        wellPanel(
          ...
          p(style = "text-align: justify", "
            ...
            Click Upload button to show the archive. You can download the archive by hitting download button."),
          br(),
  ...
  fishArchive<- eventReactive(input$archive_upload,{
    read.csv(file = "Archive.csv", header = T, fill = T)
  })
    ...
    fluidPage(
      fluidRow(
        ...
        wellPanel(
          ...
          hr(),
          p(style = "text-align: justify", "
            Here, you will be able to visualize the log data of the changes made to this database by the user. 
            Click upload button to show the latest version of the log. You can download the log sheet by hitting download button."),
          br(),
  ...
  fishlog<- eventReactive(input$log_upload,{
    read.csv(file = "log.csv", header = T, fill = T)
  })
      ...
      fluidRow(
        ...
        wellPanel(
          ...
          p(style = "text-align: justify;color:blue", strong("Instructions: "),"
            ...
            the Archive file by hitting the upload button. This will provide a quick view of Archive table and you can search your stock
            using the top left search box. Then, enter your required stock number in the field below the Archive table and hit Identify Genealogy "),
          br(),
  ...
  observeEvent(input$Genealogy_generate,{
    ...
    tree_plot<- print(plot(g,layout=co, edge.arrow.size=1, edge.curved=0.3,edge.color="black",edge.width=1.5,vertex.size=25, vertex.frame.color="black", vertex.label.color="black", vertex.color="orange",
    ...
    output$genealogy_tree<- renderPlot(
      print(plot(g,layout=co, edge.arrow.size=1,edge.color="black",edge.width=1.5,vertex.size=25, vertex.frame.color="black", vertex.label.color="black", vertex.color="orange",
                 vertex.label.cex=1))
    )
    ...
    output$Download_genealogy_table<-downloadHandler(
      filename=function(){
        paste("Genealogy tree_", input$Select_genealogy_stock, " - ", av[stoc_loc, 4],".xlsx")
      },
      content=function(file){
  ...
  output$about_ui<- renderUI({
    ...
    fluidPage(
      wellPanel(
        ...
        hr(),
        p(style="text-align: justify",em("This database is dedicated to the Mokalled Lab. Mokalled lab has fish stocks in both
                                     ...
                                     multiple tanks in the adult room with same stock number. Please try to add complete information
      ...
      wellPanel(
        ...
        hr(),
        p(style="text-align: justify", em("Each adult Fish room has predesignated rack space for each user in the Mokalled Lab. Eventhough, 
      ...
      wellPanel(
        ...
        hr(),
        p(style="text-align: justify", em("This website is currently at its very first version (v1.0.0, last updated: 17-July-2023).
          ...
          em(". You are also welcome to report any suggestions and improvements to this 
