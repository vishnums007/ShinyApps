...
server <- function(input, output, session) { 
  ...
  observeEvent(input$nursery_add_select,{
    ...
    output$nursery_room_load_success<-renderText(
      paste0("The room map ",nursery_add_choice() ," is successfully loaded")
    )
  ...
  observeEvent(input$Add_nursery_confirm, {
    ...
    if(dup_check_archive==T){
      ...
      showModal(modalDialog(
        ...
        title = strong("Confirm Overwriting Stock number"),
  ...
  output$about_ui<- renderUI({
    ...
    fluidPage(
      ...
      wellPanel(
        ...
        hr(),
        p(style="text-align: justify;font-size:16px", ("This website is currently at its very first versions (v1.0.1, last updated: 30-July-2023).
