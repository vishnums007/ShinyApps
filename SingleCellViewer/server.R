...
## creating umap
  ...
  cluster.averages<- eventReactive(input$avg1_plot_graph,{
                          ...
                          cv<-AverageExpression(data(), return.seurat = F, verbose = F, assays = "SCT", group.by = input$avg_exp_group)
                          return(cv)
  ...
  observeEvent(input$scRNA_data_upload,{
    ...
    graph_data_avg1<-eventReactive(input$avg1_plot_graph,{
      ...
      Avg_expression<-as.numeric(as.vector(cluster.averages$SCT[index,]))
      clusters<-colnames(cluster.averages$SCT)
      graph_data_clus<-reactive(data.frame(clusters, Avg_expression))
      ggplot(graph_data_clus(), aes(x=clusters, y=Avg_expression, fill= Avg_expression)) + 
  ...
  # Conserved markers between Timepoints
    ...
    output$Caution_Cons_marker<-renderText(
      paste0("Note: You are calculating conserved markers of the cluster ", input$clusters_cons ," against different timepoints and this might take upto 5 mintutes" )
    )
  ...
  genelist<- eventReactive(input$scRNA_data_upload,{
    ...
    dotplot<-  eventReactive(input$make_Dotplot,{
      if (input$Dotplot_choices =="Yes"){
        DotPlot(data(),features=rev(unique(input$Dotplot_genes)), dot.scale = 10, split.by = "Timepoint", group.by = input$Dotplot_group,cols = c("blue","blue","blue","blue"),assay = "SCT")
      }else{
        DotPlot(data(),features=rev(unique(input$Dotplot_genes)), group.by = input$Dotplot_group, assay = "SCT") + RotatedAxis()  
      }
    })
